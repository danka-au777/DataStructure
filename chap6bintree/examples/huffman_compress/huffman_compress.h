...
// 文件路径名:huffman\huffman_compress.h
...
//  哈夫曼压缩类方法声明及重载编译系统默认方法声明:
	...
	HuffmanCompress &operator=(const HuffmanCompress& copy);// 赋值运算符
};
...
// 操作结果：由哈夫曼压缩类对象copy构造新哈夫曼压缩类对象——复制构造函数
{
	...
	{	// copy为非空哈夫曼压缩类对象
		pHuffmanTree = new HuffmanTree<char, unsigned long>(*copy.pHuffmanTree);	// 生成哈夫曼树
	}
