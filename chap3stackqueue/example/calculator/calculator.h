...
// 操作结果：执行运算left op right
{
	...
	else if (theta == '/' && right != 0) result = left / right;		// 除法运算
	else if (theta == '%' && (long)right == 0) throw "除数为零!";	// 抛出异常
	else if (theta == '%' && (long)right != 0) result = (long)left % (long)right;// 整除运算
	else if (theta == '^') result = pow(left, right);	// 乘方运算
...
// 操作结果：从栈opnd中退出两个操作数
{
	if (opnd.Pop(right) != SUCCESS) throw Error("表达式有错!");		// 抛出异常
	if (opnd.Pop(left) != SUCCESS) throw Error("表达式有错!");		// 抛出异常
}
